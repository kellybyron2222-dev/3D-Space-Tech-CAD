...
### Loop J ? Cut handles + placement selection
- Cut features get viewport drag handles: move on sketch plane (offset U/V), width/height (or diameter for circle), depth along extrusion axis
- `editFeature` / `canEditHandles` include `cut`; HUD readout shows depth while dragging
- `addFeature` atomically selects the new feature (fixes hole/cut placement losing selection to undo-stack race)
